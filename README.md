# Table of Contents
- [WiiDesk Operations Manual](#wiidesk-operations-manual)
- [BEFORE READING THE REST](#before-reading-the-rest)
- [Installing WiiDesk](#installing-wiidesk)
- [WiiDesk Menu](#wiidesk-menu)
  - [To create a channel:](#to-create-a-channel)
  - [To delete a channel:](#to-delete-a-channel)
  - [To edit a channel:](#to-edit-a-channel)
  - [To move a channel:](#to-move-a-channel)
    - [Method 1](#method-1)
    - [Method 2 (recommended)](#method-2-recommended)
  - [To reset EVERYTHING (can't be undone!)](#to-reset-everything-cant-be-undone)
- [That's it folks!](#thats-it-folks)


# WiiDesk Operations Manual

Welcome to WiiDesk! You should read this before you use it.

# BEFORE READING THE REST

1. If you experience any glitches like sound not playing or stuff is missing, press ESC on your keyboard. This will open the Pause Menu

In the Pause Menu click Wii Menu and click Yes to reset

This will reset the menu and basically refresh it

2. You can change Disc Channel's target directory (default: C drive).

Changing the name of any of the default channels will reset its icon

3. WiiDesk does not open instantly when logging onto your Windows account. Takes 3-15 secs to launch

I recommend you hide your desktop icons and set your wallpaper to black to make it look like something is going to replace the desktop

4. To close WiiDesk and show your true desktop, press Ctrl+Shift+Esc

This will show the Windows Task Manager, you may terminate the WiiDesk task from there

5. Wii Settings and Mail are on the works. They don't work yet

# Installing WiiDesk

1. Go to the [latest release](https://github.com/WiiDesk/WiiDesk-1.0.0/releases/latest) & download it

2. Run `WiiDesk.Setup.1.0.0.exe`

3. That should be it

A `.bat` file will also appear for toggling WiiDesk AutoLaunch

# WiiDesk Menu

nice now you're in the menu now

The following guides will be done in the Channel Manager

## To create a channel:

1. Click the Wii button located on the bottom left

2. Click on Manage Channels

3. Click Add Channel

4. Add a name for your coolswag epic channel

5. Pick a channel icon by picking an image file from your drive (make sure it's 512x288 for the best results)

6. Upload a preview image (same as step 5)

7. Choose some music (same as step 5)

8. Choose what it'll do on open (if you leave it blank you won't be able to open it)

You can choose to make it open a web link, just make sure you include `https://` at the start

For an app you must find your program's executable path and paste it in

There is no limit to how many channels you create

## To delete a channel:

1. Hover over the channel

2. Click the red X (**can't be undone!**)

## To edit a channel:

1. Hover over the channel

2. Click the blue pencil

## To move a channel:

### Method 1

1. Hover over a channel

2. Click the left/right arrows to move it one slot to the left/right respectively

### Method 2 (recommended)

1. Drag the channel to the place of an existing channel

Both channels will swap slots

## To reset EVERYTHING (can't be undone!)

1. Click the Reset button in Channel Manager

# That's it folks!

nintendo don't sue me pls
